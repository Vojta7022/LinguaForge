...
function buildFallbackUnit(
  ...
  const lessons: LessonDefinition[] = [
    ...
    },
    {
      ...
      description: 'Mix the new words and grammar in quick recall exercises.',
      icon: lessonIcon('skill_practice', topic),
      topic: `${topic} mixed practice`,
      lessonKind: 'skill_practice',
      skillType: 'mixed',
      focusLabel: LESSON_KIND_META.skill_practice.focusLabel,
      objective: `Use ${vocabularyFocus[1]} and ${grammarFocus[0]} together in ${topic.toLowerCase()} contexts.`,
      grammarFocus,
      vocabularyFocus,
      canDo,
    },
